...
void TrainingWriteBuffers::addRow(
  ...
  if(posHistForFutureBoards == NULL) {
    ...
    const Board& board2 = boards[min(whiteValueTargetsIdx+8,endIdx)];
    const Board& board3 = boards[min(whiteValueTargetsIdx+32,endIdx)];
...
bool TrainingDataWriter::flushIfNonempty(string& resultingFilename) {
  ...
  if(debugOut != NULL) {
    ...
    writeBuffers->clear();
    rename(tmpFilename.c_str(),resultingFilename.c_str());
  }
  return true;
}
