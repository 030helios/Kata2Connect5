...
static double finalScoreIfGameEndedNow(const BoardHistory &baseHist, const Board &baseBoard)
{
  ...
  {
    hist.makeBoardMoveAssumeLegal(board, Board::PASS_LOC, Board::PASS_LOC, pla, NULL);
    pla = getOpp(pla);
  }
...
void Tests::runNNInputsV3V4Tests()
{
...

  {
    ...
    for (int version = minVersion; version <= maxVersion; version++)
    {
      ...
      {
        hist.makeBoardMoveAssumeLegal(board, moves[i].fromLoc, moves[i].toLoc, moves[i].pla, NULL);
            ...
            for (int x = 0; x < board.x_size; x++)
            {
              for (int i = 0; i < board.x_size; i++)
              {
                for (int j = 0; j < board.y_size; j++)
                {
                  ...
                  out << (int)hist.isLegal(board, from, to, nextPla);
                }
              }
            }
            out << " ";
...
o.xoo.x
          ...
          auto runBoth = [&]()
          {
            ...
            for (int i = 0; i < hist.moveHistory.size(); i++)
              cout << Location::toString(hist.moveHistory[i].fromLoc, board) << " "<< Location::toString(hist.moveHistory[i].toLoc, board) << " ";
            cout << actualNHWC;
            printScoring();
          };
