    ...
    double &desiredDynamicPDAForWhite)
{
  ...
  else
  {
    ...
    double pdaScalingStartPoints = max(4.0 / boardSizeScaling, 2.0);
    ...
    else
    {
      ...
      else
      {
        ...
        desiredDynamicPDAForDisadvantagedPla = max(desiredDynamicPDAForDisadvantagedPla, 0.0);
        desiredDynamicPDAForDisadvantagedPla = min(desiredDynamicPDAForDisadvantagedPla, pdaCap);
      }
...
struct GTPEngine
{
  ...
  bool setPosition(const vector<Move> &initialStones)
  {
    ...
    for (int i = 0; i < initialStones.size(); i++)
      board.colors[initialStones[i].toLoc] = initialStones[i].pla;
    Player pla = P_BLACK;
    BoardHistory hist(board, pla, currentRules, 0);
  ...
  bool play(Loc From, Loc to, Player pla)
  {
    assert(bot->getRootHist().rules == currentRules);
    bool suc = bot->makeMove(From, to, pla);
    if (suc)
      moveHistory.push_back(Move(From, to, pla));
    return suc;
  }
  ...
  bool undo()
  {
    ...
    {
      bool suc = play(moveHistoryCopy[i].fromLoc, moveHistoryCopy[i].toLoc, moveHistoryCopy[i].pla);
      assert(suc);
  ...
  function<void(const Search *search)> getAnalyzeCallback(Player pla, AnalyzeArgs args)
  {
    function<void(const Search *search)> callback;
    //lz-analyze
    if (args.lz && !args.kata)
    {
      ...
      callback = [args, pla, this](const Search *search)
      {
        ...
        for (int i = 0; i < buf.size(); i++)
        {
          ...
          cout << "info";
          cout << " from " << Location::toString(data.fromLoc, board);
          cout << " to " << Location::toString(data.toLoc, board);
      ...
      AnalyzeArgs args)
  {
    ...
    else
    {
      double desiredDynamicPDA =
          (params.playoutDoublingAdvantagePla == P_WHITE) ? desiredDynamicPDAForWhite : (params.playoutDoublingAdvantagePla == P_BLACK)                 ? -desiredDynamicPDAForWhite
    ...
    if (args.analyzing)
    {
      function<void(const Search *search)> callback = getAnalyzeCallback(pla, args);
      if (args.showOwnership)
        bot->setAlwaysIncludeOwnerMap(true);
      else
        bot->setAlwaysIncludeOwnerMap(false);
      mov = bot->genMoveSynchronousAnalyze(pla, tc, searchFactor, args.secondsPerReport, callback);
      //Make sure callback happens at least once
      callback(bot->getSearch());
    }
    else
    {
      mov = bot->genMoveSynchronous(pla, tc, searchFactor);
    }

    bool isLegal = bot->search->rootHistory.isLegal(bot->search->rootBoard, mov.fromLoc, mov.toLoc, pla);
    if (mov.fromLoc == Board::NULL_LOC || mov.toLoc == Board::NULL_LOC || !isLegal)
    {
      ...
      sout << "from: " << Location::toString(mov.fromLoc, bot->getRootBoard()) << "\n";
      sout << "to: " << Location::toString(mov.toLoc, bot->getRootBoard()) << "\n";
    ...
    if (ogsChatToStderr)
    {
      ...
      if (params.playoutDoublingAdvantage != 0.0)
      {
        cerr << Global::strprintf(
            " (PDA %.2f)",
            bot->getSearch()->getRootPla() == getOpp(params.playoutDoublingAdvantagePla) ? -params.playoutDoublingAdvantage : params.playoutDoublingAdvantage);
      }
      cerr << " PV ";
      bot->getSearch()->printPVForMove(cerr, bot->getSearch()->rootNode, mov.fromLoc, mov.toLoc, analysisPVLen);
      cerr << endl;
    }
    ...
    {
      ostringstream sout;
      PlayUtils::printGenmoveLog(sout, bot, nnEval, mov.fromLoc, mov.toLoc, timeTaken, perspective);
      logger.write(sout.str());
    }
    if (debug)
    {
      PlayUtils::printGenmoveLog(cerr, bot, nnEval, mov.fromLoc, mov.toLoc, timeTaken, perspective);
    }
    ...
    {
      response = Location::toString(mov.fromLoc, bot->getRootBoard());
      response += " ";
      response += Location::toString(mov.toLoc, bot->getRootBoard());
    }

    if (!resigned && mov.fromLoc != Board::NULL_LOC && mov.toLoc != Board::NULL_LOC && isLegal && playChosenMove)
    {
      bool suc = bot->makeMove(mov.fromLoc, mov.toLoc, pla);
      if (suc)
        moveHistory.push_back(mov);
      assert(suc);
      (void)suc; //Avoid warning when asserts are off
  ...
  void computeAnticipatedWinnerAndScore(Logger &logger, Player &winner, double &finalWhiteMinusBlackScore)
  {
    ...
    {
      int64_t numVisits = max(50, params.numThreads * 10);
      ...
      winner = lead > 0 ? P_WHITE : lead < 0 ? P_BLACK
                                             : C_EMPTY;
    }
  ...
  vector<bool> computeAnticipatedStatuses(Logger &logger)
  {
    ...
    int64_t numVisits = max(100, params.numThreads * 20);
    vector<bool> isAlive;
    ...
    for (int symmetry = 0; symmetry < NNInputs::NUM_SYMMETRY_COMBINATIONS; symmetry++)
    {
      if (whichSymmetry == NNInputs::SYMMETRY_ALL || whichSymmetry == symmetry)
      {
        ...
        for (int y = 0; y < board.y_size; y++)
        {
          for (int x = 0; x < board.x_size; x++)
          {
            int to = Location::getLoc(x, y, 6);
            int from = Location::getLoc(0, 0, 6);
            int pos = NNPos::locToDoublePos(from, to, board.x_size, 6, 6);
    ...
    GTPEngine *engine)
{
  ...
  while (pieces.size() > numArgsParsed)
  {
    ...
    else if (key == "avoid" || key == "allow")
    {
      ...
      if (key == "allow")
      {
        fill(avoidMoveUntilByLoc.begin(), avoidMoveUntilByLoc.end(), untilDepth);
...
int MainCmds::gtp(int argc, const char *const *argv)
{
  ...
  const int defaultBoardXSize =
      cfg.contains("defaultBoardXSize") ? cfg.getInt("defaultBoardXSize", 2, Board::MAX_LEN) : cfg.contains("defaultBoardSize") ? cfg.getInt("defaultBoardSize", 2, Board::MAX_LEN)
                                                                                                                                : -1;
  const int defaultBoardYSize =
      cfg.contains("defaultBoardYSize") ? cfg.getInt("defaultBoardYSize", 2, Board::MAX_LEN) : cfg.contains("defaultBoardSize") ? cfg.getInt("defaultBoardSize", 2, Board::MAX_LEN)
                                                                                                                                : -1;
  ...
  const double genmoveAntiMirror =
      cfg.contains("genmoveAntiMirror") ? cfg.getBool("genmoveAntiMirror") : cfg.contains("antiMirror") ? cfg.getBool("antiMirror")
                                                                                                        : true;
  ...
  while (getline(cin, line))
  {
    ...
    else if (command == "known_command")
    {
      ...
      {
        if (find(knownCommands.begin(), knownCommands.end(), pieces[0]) != knownCommands.end())
          response = "true";
        else
          response = "false";
      }
    ...
    else if (command == "time_left")
    {
      ...
      double time;
      int stones;
      if (pieces.size() != 3 || !PlayerIO::tryParsePlayer(pieces[0], pla) || !Global::tryStringToDouble(pieces[1], time) || !Global::tryStringToInt(pieces[2], stones))
      {
        responseIsError = true;
        ...
        else
        {
          ...
          else
          {
            ...
            if (tc.originalNumPeriods > 1 && tc.numStonesPerPeriod == 1)
            {
              tc.mainTimeLeft = 0.0;
              tc.inOvertime = true;
              tc.numPeriodsLeftIncludingCurrent = min(stones, tc.originalNumPeriods);
            ...
            else
            {
              ...
              tc.numStonesLeftInPeriod = min(stones, tc.numStonesPerPeriod);
              tc.timeLeftInPeriod = time;
            }
          }
          if (pla == P_BLACK)
    ...
    else if (command == "play")
    {
      ...
      else if (!tryParseLoc(pieces[1], engine->bot->getRootBoard(), fromLoc))
      {
        responseIsError = true;
        response = "Could not parse vertex: '" + pieces[1] + "'";
      }
      else if (!tryParseLoc(pieces[1], engine->bot->getRootBoard(), toLoc))
      {
        responseIsError = true;
        response = "Could not parse vertex: '" + pieces[1] + "'";
      }
      ...
      {
        bool suc = engine->play(fromLoc, toLoc, pla);
        if (!suc)
        {
          responseIsError = true;
    ...
    else if (command == "set_position")
    {
      ...
      else
      {
        ...
        for (int i = 0; i < pieces.size(); i += 2)
        {
          ...
          initialStones.push_back(Move(0, loc, pla));
        }
        if (!responseIsError)
        {
