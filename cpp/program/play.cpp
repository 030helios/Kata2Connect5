...
void GameInitializer::initShared(ConfigParser &cfg, Logger &logger)
{
  ...
  auto generateCumProbs = [](const vector<Sgf::PositionSample> poses, double lambda, double &effectiveSampleSize)
  {
    ...
    for (size_t i = 0; i < poses.size(); i++)
      minInitialTurnNumber = min(minInitialTurnNumber, poses[i].initialTurnNumber);
  ...
  if (cfg.contains("startPosesFromSgfDir"))
  {
    ...
    vector<string> files;
    function<bool(const string &)> fileFilter = [](const string &fileName)
    {
    ...
    set<Hash128> excludeHashes = Sgf::readExcludes(excludes);
    ...
    function<void(Sgf::PositionSample &, const BoardHistory &, const string &)> posHandler = [startPosesLoadProb, this](
    ...
    const Sgf::PositionSample *startPosSample)
{
  ...
  lock_guard<mutex> lock(createGameMutex);
...
vector<int> GameInitializer::getAllowedBSizes() const
{
    ...
    const Sgf::PositionSample *startPosSample)
{
  ...
  if (asymmetricProb > 0 && rand.nextBool(asymmetricProb))
  {
    ...
    makeGameFairProb = max(makeGameFairProb, playSettings.minAsymmetricCompensateKomiProb);
  }
...
static void failIllegalMove(Search *bot, Logger &logger, Board board, Loc from, Loc to)
{
  ostringstream sout;
  ...
  sout << "from: " << Location::toString(from, bot->getRootBoard()) << "\n";
  sout << "to: " << Location::toString(to, bot->getRootBoard()) << "\n";
  logger.write(sout.str());
  bot->getRootBoard().checkConsistency();
  ASSERT_UNREACHABLE;
}

static void logSearch(Search *bot, Logger &logger, Loc from, Loc to, OtherGameProperties otherGameProps)
{
  ostringstream sout;
  Board::printBoard(sout, bot->getRootBoard(), from, &(bot->getRootHist().moveHistory));
  Board::printBoard(sout, bot->getRootBoard(), to, &(bot->getRootHist().moveHistory));
  sout << "\n";
  ...
  {
    sout << "HintLoc " << Location::toString(otherGameProps.hintMove.fromLoc, bot->getRootBoard()) << " " << Location::toString(otherGameProps.hintMove.toLoc, bot->getRootBoard()) << "\n";
  }
...
static Move chooseRandomForkingMove(const NNOutput *nnOutput, const Board &board, const BoardHistory &hist, Player pla, Rand &gameRand, Move banMove)
{
    ...
    vector<double> &playSelectionValuesBuf)
{
  ...
  bool success = toMoveBot->getPlaySelectionValues(*node, FromlocsBuf, TolocsBuf, playSelectionValuesBuf, NULL, scaleMaxToAtLeast, allowDirectPolicyMoves);
  assert(success);
  (void)success; //Avoid warning when asserts are disabled

  //Make sure we don't overflow int16
  double maxValue = 0.0;
  for (int moveIdx = 0; moveIdx < FromlocsBuf.size(); moveIdx++)
  {
  ...
  for (int moveIdx = 0; moveIdx < FromlocsBuf.size(); moveIdx + 2)
  {
    double value = playSelectionValuesBuf[moveIdx] * factor;
    assert(value <= 30001.0);
    buf.push_back(PolicyTargetMove(FromlocsBuf[moveIdx], TolocsBuf[moveIdx], (int16_t)round(value)));
  }
}
    ...
    Move excludeLoc0, Move excludeLoc1)
{
  ...
  if (plaAlwaysBest && node != toMoveBot->rootNode)
  {
    ...
    extractPolicyTarget(sp->policyTarget, toMoveBot, node, FromlocsBuf, TolocsBuf, playSelectionValuesBuf);
  ...
  for (int i = 0; i < node->numChildren; i++)
  {
    ...
    const SearchNode *child = node->children[i];
    if ((child->prevFromLoc == excludeLoc0.fromLoc && child->prevToLoc == excludeLoc0.toLoc) || (child->prevFromLoc == excludeLoc1.fromLoc && child->prevToLoc == excludeLoc1.toLoc))
      continue;
    ...
    histCopy.makeBoardMoveAssumeLegal(copy, child->prevFromLoc, child->prevToLoc, pla, NULL);
    ...
    recordTreePositionsRec(
        ...
        FromlocsBuf, TolocsBuf, playSelectionValuesBuf,
    ...
    Move excludeLoc0, Move excludeLoc1)
{
  ...
  recordTreePositionsRec(
      ...
      minVisitsAtNode, recordTreeTargetWeight,
      numNeuralNetChangesSoFar, FromlocsBuf,
      TolocsBuf, playSelectionValuesBuf,
    ...
    const OtherGameProperties &otherGameProps)
{
  ...
  if (otherGameProps.hintMove.fromLoc != Board::NULL_LOC && otherGameProps.hintMove.toLoc != Board::NULL_LOC )
  {
        ...
        otherGameProps.hintPosHash == toMoveBot->getRootBoard().pos_hash)
    {
      ...
      numAlterVisits = (int64_t)ceil(min(cap, numAlterVisits * 4.0));
      numAlterPlayouts = (int64_t)ceil(min(cap, numAlterPlayouts * 4.0));
    }
  }
  ...
  if ((otherGameProps.hintMove.fromLoc == Board::NULL_LOC || otherGameProps.hintMove.toLoc == Board::NULL_LOC ) && cheapSearchProb > 0.0 && gameRand.nextBool(cheapSearchProb))
  {
    ...
    doAlterVisitsPlayouts = true;
    numAlterVisits = min(numAlterVisits, (int64_t)playSettings.cheapSearchVisits);
    numAlterPlayouts = min(numAlterPlayouts, (int64_t)playSettings.cheapSearchVisits);
  ...
  else if ((otherGameProps.hintMove.fromLoc == Board::NULL_LOC || otherGameProps.hintMove.toLoc == Board::NULL_LOC ) && playSettings.reduceVisits)
  {
    ...
    if (historicalMctsWinLossValues.size() >= playSettings.reduceVisitsThresholdLookback)
    {
      ...
      double signedMostExtreme = max(minWinLossValue, -maxWinLossValue);
      ...
      if (amountThrough > 0)
      {
        ...
        numAlterVisits = max(numAlterVisits, (int64_t)playSettings.reducedVisitsMin);
        numAlterPlayouts = max(numAlterPlayouts, (int64_t)playSettings.reducedVisitsMin);
      }
    }
  }
    ...
    Logger &logger)
{
  ...
  if (limits.doAlterVisitsPlayouts)
  {
    ...
    if ((limits.hintMove.fromLoc != Board::NULL_LOC &&limits.hintMove.toLoc != Board::NULL_LOC ))
    {
      ...
      toMoveBot->setRootHintLoc(limits.hintMove);
    }
...
FinishedGameData *Play::runGame(
    ...
    int maxMovesPerGame, const function<bool()> &shouldStop,
    const PlaySettings &playSettings, const OtherGameProperties &otherGameProps,
    Rand &gameRand,
    function<NNEvaluator *()> checkForNewNNEval,
    function<void(const Board &, const BoardHistory &, Player, Loc, const vector<double> &, const vector<double> &, const vector<double> &, const Search *)> onEachMove)
{
  Search *botB;
  Search *botW;
...
FinishedGameData *Play::runGame(
    ...
    Rand &gameRand,
    function<NNEvaluator *()> checkForNewNNEval,
    function<void(const Board &, const BoardHistory &, Player, Move, const vector<double> &, const vector<double> &, const vector<double> &, const Search *)> onEachMove)
{
  ...
  for (int i = 0; i < maxMovesPerGame; i++)
  {
    ...
    if (mov.fromLoc == Board::NULL_LOC || mov.toLoc == Board::NULL_LOC || !toMoveBot->rootHistory.isLegal(board, mov.fromLoc, mov.toLoc, pla))
      failIllegalMove(toMoveBot, logger, board, mov.fromLoc, mov.toLoc);
    if (logSearchInfo)
      logSearch(toMoveBot, logger, mov.fromLoc, mov.toLoc, otherGameProps);
    if (logMoves)
      logger.write("Move " + Global::uint64ToString(hist.moveHistory.size()) + " from: " + Location::toString(mov.fromLoc, board) + " to: " + Location::toString(mov.toLoc, board));
    ...
    else
    {
      ...
      Move sidePositionForkMove = Move(0, 0, 0);
      if (playSettings.sidePositionProb > 0.0 && gameRand.nextBool(playSettings.sidePositionProb))
      {
        ...
        Move banMove = mov;
        sidePositionForkMove = chooseRandomForkingMove(toMoveBot->rootNode->nnOutput.get(), board, hist, pla, gameRand, banMove);
        if (sidePositionForkMove.fromLoc != Board::NULL_LOC && sidePositionForkMove.toLoc != Board::NULL_LOC)
        {
          SidePosition *sp = new SidePosition(board, hist, pla, gameData->changedNeuralNets.size());
          sp->hist.makeBoardMoveAssumeLegal(sp->board, sidePositionForkMove.fromLoc, sidePositionForkMove.toLoc, sp->pla, NULL);
    ...
    if (onEachMove != nullptr)
      onEachMove(board, hist, pla, mov, historicalMctsWinLossValues, historicalMctsLeads, historicalMctsScoreStdevs, toMoveBot);

    //Finally, make the move on the bots
    bool suc;
    suc = botB->makeMove(mov.fromLoc, mov.toLoc, pla);
    assert(suc);
    if (botB != botW)
    {
      suc = botW->makeMove(mov.fromLoc, mov.toLoc, pla);
      assert(suc);
    }
    ...
    if (playSettings.allowResignation && historicalMctsWinLossValues.size() >= playSettings.resignConsecTurns)
    {
      ...
      {
        if (playSettings.resignThreshold > 0 || isnan(playSettings.resignThreshold))
          throw StringError("playSettings.resignThreshold > 0 || isnan(playSettings.resignThreshold)");
  ...
  if (recordFullData)
  {
    ...
    if (hist.isGameFinished && hist.isNoResult)
    {
      ...
      fill(gameData->finalFullArea, gameData->finalFullArea + Board::MAX_ARR_SIZE, C_EMPTY);
      fill(gameData->finalOwnership, gameData->finalOwnership + Board::MAX_ARR_SIZE, C_EMPTY);
      fill(gameData->finalSekiAreas, gameData->finalSekiAreas + Board::MAX_ARR_SIZE, false);
    }
    else
    {
      ...
      finalValueTargets.score = (float)ScoreValue::whiteScoreDrawAdjust(hist.finalWhiteMinusBlackScore);
    ...
    if ((otherGameProps.hintMove.fromLoc != Board::NULL_LOC &&otherGameProps.hintMove.toLoc != Board::NULL_LOC ))
    {
      gameData->whiteValueTargetsByTurn[0] = gameData->whiteValueTargetsByTurn[min((size_t)1, gameData->whiteValueTargetsByTurn.size() - 1)];
    }
    ...
    vector<double> valueSurpriseByTurn;
    {
      ...
      for (int i = rawNNValues.size() - 1; i >= 0; i--)
      {
        ...
        if (winValue > 1e-100)
          valueSurprise += winValue * (log(winValue) - log(max((double)rawNNValues[i].winValue, 1e-100)));
        if (lossValue > 1e-100)
          valueSurprise += lossValue * (log(lossValue) - log(max((double)rawNNValues[i].lossValue, 1e-100)));
        if (noResultValue > 1e-100)
          valueSurprise += noResultValue * (log(noResultValue) - log(max((double)rawNNValues[i].noResultValue, 1e-100)));
        ...
        valueSurpriseByTurn[i] = min(valueSurprise, 1.0);
      }
    }
    ...
    if (playSettings.policySurpriseDataWeight > 0 || playSettings.valueSurpriseDataWeight > 0)
    {
      ...
      if (sumWeights >= 1)
      {
        ...
        if (averageValueSurpriseWeighted < 0.010)
        { //0.010 logits on average, pretty arbitrary, mainly just intended limit to extreme cases.
        ...
        for (int i = 0; i < numWeights; i++)
        {
          ...
          double policySurprisePropValue =
              targetWeight * policySurprise + (1 - targetWeight) * max(0.0, policySurprise - thresholdToIncludeReduced);
        ...
        sumPolicySurprisePropValue = max(sumPolicySurprisePropValue, 1e-10);
        sumValueSurprisePropValue = max(sumValueSurprisePropValue, 1e-10);
        ...
        for (int i = 0; i < numWeights; i++)
        {
          ...
          double valueSurprisePropValue =
              targetWeight * valueSurprise;
          double newValue =
              (1.0 - playSettings.policySurpriseDataWeight - valueSurpriseDataWeight) * targetWeight + playSettings.policySurpriseDataWeight * policySurprisePropValue * sumWeights / sumPolicySurprisePropValue + valueSurpriseDataWeight * valueSurprisePropValue * sumWeights / sumValueSurprisePropValue;
    ...
    for (int i = 0; i < sidePositionsToSearch.size(); i++)
    {
      ...
      Move responseMove = toMoveBot->runWholeSearchAndGetMove(sp->pla, logger);

      extractPolicyTarget(sp->policyTarget, toMoveBot, toMoveBot->rootNode, FromlocsBuf, TolocsBuf, playSelectionValuesBuf);
      ...
      {
        if (responseMove.fromLoc == 0 || responseMove.toLoc == 0 || !sp->hist.isLegal(sp->board, responseMove.fromLoc, responseMove.toLoc, sp->pla))
          failIllegalMove(toMoveBot, logger, sp->board, responseMove.fromLoc, responseMove.toLoc);

        SidePosition *sp2 = new SidePosition(sp->board, sp->hist, sp->pla, gameData->changedNeuralNets.size());
        sp2->hist.makeBoardMoveAssumeLegal(sp2->board, responseMove.fromLoc, responseMove.toLoc, sp2->pla, NULL);
        ...
        else
        {
          ...
          Move banMove = Move(0, 0, 0);
          Move forkMove = chooseRandomForkingMove(nnResultBuf.result.get(), sp2->board, sp2->hist, sp2->pla, gameRand, banMove);
          if (forkMove.fromLoc != Board::NULL_LOC || forkMove.toLoc != Board::NULL_LOC)
          {
            sp2->hist.makeBoardMoveAssumeLegal(sp2->board, forkMove.fromLoc, forkMove.toLoc, sp2->pla, NULL);
            sp2->pla = getOpp(sp2->pla);
            if (sp2->hist.isGameFinished)
              delete sp2;
            else
...
static void replayGameUpToMove(const FinishedGameData *finishedGameData, int moveIdx, Rules rules, Board &board, BoardHistory &hist, Player &pla)
{
  ...
  if (finishedGameData->endHist.moveHistory.size() <= 0)
    return;
  moveIdx = min(moveIdx, (int)(finishedGameData->endHist.moveHistory.size() - 1));

  //Replay all those moves
  for (int i = 0; i < moveIdx; i++)
  {
    Move mov = finishedGameData->endHist.moveHistory[i];
    if (!hist.isLegal(board, mov.fromLoc, mov.toLoc, pla))
    {
      ...
      if (rules == finishedGameData->startHist.rules)
      {
        ...
        cout << Location::toString(mov.fromLoc, board) << endl;
        cout << Location::toString(mov.toLoc, board) << endl;
    ...
    Search *bot)
{
  ...
  {
    moveIdx = (int)floor(
        gameRand.nextExponential() * (playSettings.earlyForkGameExpectedMoveProp * finishedGameData->startBoard.x_size * finishedGameData->startBoard.y_size));
  }
  else if (lateFork)
  {
  ...
  Move possibleMoves[NNPos::MAX_NN_POLICY_SIZE];
  ...
  {
    Move mov = possibleMoves[i];
    Board copy = board;
    BoardHistory copyHist = hist;
    copyHist.makeBoardMoveAssumeLegal(copy, mov.fromLoc, mov.toLoc, pla, NULL);
    MiscNNInputParams nnInputParams;
    ...
    if (bestMove.fromLoc == Board::NULL_LOC || bestMove.toLoc == Board::NULL_LOC || (pla == P_WHITE && whiteScore > bestScore) || (pla == P_BLACK && whiteScore < bestScore))
    {
      bestMove = mov;
  ...
  //Make that move
  assert(hist.isLegal(board, bestMove.fromLoc, bestMove.toLoc, pla));
  hist.makeBoardMoveAssumeLegal(board, bestMove.fromLoc, bestMove.toLoc, pla, NULL);
  pla = getOpp(pla);
